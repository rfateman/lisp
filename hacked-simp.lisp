...
(defun simptimes (x w z)		; W must be 1
  ...
  st1
     ...
     ;;(setq res (tms (copy-tree w) 1 (copy-tree res)))
    ;; (format t "~% w=~s" w)
     ;;  (setq res (tms (copy-tree w) 1 res)) ;; only one copy-tree,copy-list no good
       (setq res (tms w  1 res))  ;; this breaks it, I think. 1/18/23 RJF
...
(defun checkvar1(where)
  (declare (special $var1 $var1copy $checkit))
  (if (and $checkit (not (equal $var1 $var1copy))) (format t "~%broken in ~s, ~s" where $var1)))
(defun checkvar1-break(where)
  (declare (special $var1 $var1copy $checkit))
  (cond ((and $checkit (not (equal $var1 $var1copy)))
	 (format t "~%broken in ~s, ~s" where $var1)
	 (break))))
...
(defun subexpression-matches-p (predicate x)
       (checkvar1 'subexpression-matches-p)
...
(defun testt (x)
  ...
  (cond ((mnump x)
	...
	((eql 1 (cadr x))
	 ...
	 (cond ((null (cdddr x)) ;;; something like  ((mtimes) 1 foo)
		(caddr x));; third item 
	       (t
		;;(rplacd x (cddr x));; changes ((mtimes) 1 foo bar) to ((mtimes) foo bar);
		(cons (car x)(cddr x)) ;; safer. 1/17/23 rjf
